...
class RecordClient:
    ...
    def run(self):
        ...
        try:
            ...
            print(f"\n{'='*60}")
            print(f"[Client] ✅ 录制完成！共 {self.episode_count} 个 episodes")
            print(f"[Client] 📁 数据集位置: {self.root}")
            print(f"\n🚀 下一步：训练模型")
            print(f"{'='*60}")
            print(f"conda activate lerobot")
            print(f"cd D:\\work\\lerobot-workspace\\lekiwi-pi")
            print(f"lerobot-train `")
            print(f"  --policy.type=act `")
            print(f"  --policy.push_to_hub=false `")
            print(f"  --dataset.repo_id=acelan `")
            print(f"  --dataset.root=\"{self.root}\" `")
            print(f"  --batch_size=16 `")
            print(f"  --steps=20000 `")
            print(f"  --num_workers=4 `")
            print(f"  --save_freq=2000 `")
            print(f"  --log_freq=100 `")
            print(f"  --output_dir=\"outputs/train/grasp_paper_ball\" `")
            print(f"  --job_name=\"grasp paper ball\" `")
            print(f"  --wandb.enable=true `")
            print(f"  --wandb.project=so101_pick")
            print(f"{'='*60}")
